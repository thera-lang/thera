...
| `type-map-bracket`            | Collections               | bracket map literals `[k: v, …]` / empty `[:]`: unrestricted keys, valid in any expression position (incl. match arms); `[]` under a `Map` expectation is the empty map                                                                         | ✓      |
...
| `bare-variant-construction`   | Bare variant construction | bare `Variant(args)` where the expected type pins the enum (return incl. implicit-`Ok` payload, throw, argument, annotated let, assignment, match-arm value); a real binding wins; arity/payload checked; qualified required where nothing pins | ✓      |
...
| `name-keyword-members`        | Keywords as member names | any keyword is a member name (field decl, struct-literal field, `.` access, call label); binding names stay identifiers — a parameter is a binding, so a keyword parameter name is a prescriptive error | ✓      |
...
| `fn-named-params`            | Named parameters          | positional by default; `named` opts into a call-site label, which _is_ the parameter's name (no two-name form); `named` is contextual, and the retired `_` marker is rejected | ✓      |
| `fn-named-params-suffix`     | Named parameters          | `named` parameters must be a suffix of the parameter list — a positional one after a `named` one is an error                                                                  | ✓      |
| `fn-arg-eval-order`          | Argument evaluation order | arguments evaluate left to right in **source** order, not parameter order (so labeled arguments are safe to reorder); omitted defaults materialize after, in parameter order  | ⓧ      |
...
| `fn-argument-label-required` | Named parameters          | passing a `named` parameter positionally is an **error** carrying a mechanical fix; not suppressable (errors have no rule); an omitted defaulted argument is exempt           | ✓      |
...
| `warn-unreachable-arm`   | Control flow / Warnings | an arm after a catch-all (incl. full coverage), a re-matched variant, or a repeated literal is a **warning**; refutable constructor arms claim nothing | ✓      |
| `warn-unreachable-code`  | Control flow / Warnings | code after a statement that always transfers control (`return`/`throw`/`break`/`continue`, all-exiting `if`) is a **warning**, once per block          | ✓      |
...
| `iface-enum-name`          | Inherent methods      | every enum value has a built-in `name()` (its variant's name); a declared `name` method wins over it                                                                                                                            | ✓      |
...
| `iface-debug`              | Display and Debug     | structural `Debug` derive for structs; `.debug()` is a declared method on every built-in (primitives, String quoted, Bytes, collections/wrappers), agreeing with nested structural rendering and honoring nested explicit impls | ◐      |
...
| `gen-param-methods`        | Interfaces / Dispatch | a method call on a type parameter resolves against its bounds; `to_string`/`debug` are universal, but any other method on an unbounded `T` is a `check` error (matching codegen)                                                | ✓      |
...
| `generic-primitive-bounds` | Bounds                | primitives satisfy exactly `Eq`/`Debug`/`Display`/`Ord`; a bound on any other interface — even one with a declared impl on the primitive — is a `check` error naming the limitation, while static calls of that impl stay legal | ✓      |
...
| `mod-ns-file-local`         | language.md          | a namespace is file-local; qualifying with one this file didn't import is rejected — values (`undefined name`) and type positions (`unknown namespace`, with a qualify-as hint; the type resolves Unknown so no same-name mismatch cascades)         | ✓      |
...
| `mod-import-self`           | Imports              | a direct self-import is rejected at the import decl (its own names are already in scope; cross-library cycles are `mod-import-cycle`)                                                                                                                | ✓      |
| `mod-import-cycle`          | Imports              | imports between libraries must be acyclic: every cross-library import inside a cycle is an error at its own decl, carrying one full cycle path (a test file's imports don't participate — it is a consumer of the libraries it imports; unit-tested) | ✓      |
| `mod-import-cycle-sibling`  | Imports              | the acyclicity unit is the library, not the file: sibling files inside one directory library may import each other freely                                                                                                                            | ✓      |
| `mod-import-deep`           | Imports              | a directory library's non-barrel files are importable only from inside that directory — an outside import of an internal is an error at the import decl; the barrel is the library's one API                                                         | ✓      |
| `mod-import-barrel-file`    | Imports              | the barrel reached by its file path is the library itself (`import 'lib'` ≡ `import 'lib/lib'`), never a deep import                                                                                                                                 | ✓      |
| `warn-unused-import`        | Imports / Warnings   | an import never referenced is a **warning** — a namespace import via its namespace, an `as _` import via its surface names (or the namespace, qualified); `// ignore: unused-import` suppresses per site                                             | ✓      |
