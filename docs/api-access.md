...
Against the checklist above, as of the typed-JSON arc:
...
| Construction and config        | ❌ no `Client` exists                                          |
| Errors                         | ❌ no error body is decoded, nothing composes with `HttpError` |
| Streaming surface              | ❌ iterator vs channel vs callback untouched                   |
| Naming (flat vs resource tree) | ❌ **no operation function exists at all**                     |
| Pagination                     | ❌ Messages does not paginate                                  |

The gap that matters most is the last-but-one: `pkgs/anthropic` is types and
codecs with **no call surface**. The generator's highest-volume output is the
operation function — `client.messages.create(…)` versus
`anthropic.create_message(client, …)` — and there is currently not one of them
to look at.

The JSON side has a parallel gap: six mapping-table rows are validated, and the
three that are not (`allOf`, `additionalProperties: {T}`, untagged `anyOf`) need
a spec that uses them. So does 3.0's `nullable: true`, since Anthropic and
OpenAI are 3.1 and spell nullability the other way, and so does `format` (12,200
occurrences in GitHub, none exercised here).

**Two milestones, then the review.**

1. **Anthropic, end to end** — `Client`, auth from the environment, the error
   body, a real call under hermetic TLS, and streaming Messages. Closes
   construction, errors, streaming, and naming. Also completes Arc 3's
   acceptance test, which is defined as regenerating Anthropic and diffing
   against this.
2. **A deliberately tiny GitHub client** — three operations (list PRs, create
   PR, create issue), whose closure is **28 schemas**. Chosen not for its own
   sake but because it is the cheapest way to cover what Anthropic structurally
   cannot: pagination, `nullable: true`, `format`/`date-time`, and a second API
   shape to compare against so the review is not generalizing from one vendor's
   house style.
...
5. **Review the hand-written shape** before anything is generated — see Arc 2 §
   When the shape is ready to evaluate. It gates step 6, and it needs the
   Anthropic client finished plus a small second client (GitHub) for the
   decisions Anthropic structurally cannot exercise.
6. **Arc 3 v1**, acceptance-tested by regenerating Anthropic and diffing against
   step 2.
7. **Second and third generated targets** — Gemini, then GitHub at full size.
