...
After a successful `bin/build_sdk.sh` (the fixpoint passed), refresh the
snapshot so it tracks `main`:
...
change so the _current_ snapshot still compiles it, rebuild, refresh the
