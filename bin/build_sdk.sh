#!/usr/bin/env bash
#
# Build the binary Thera SDK: the runtime with the compiled front-end embedded,
# plus the standard library sources. The artifact lands in build/sdk/:
#
#   build/sdk/
#     bin/thera    ← bare runtime + embedded frontend.thera-bc
...
#   1. cargo build the runtime with that blob embedded (THERA_FRONTEND_BC)
...
( cd "$ROOT/runtime" && THERA_FRONTEND_BC="$FRONTEND_BC" cargo build $CARGO_PROFILE_FLAG )

echo "==> [2/3] assembling build/sdk"
rm -rf "$SDK"
mkdir -p "$SDK/bin"
cp "$ROOT/runtime/target/$TARGET_SUBDIR/thera-rt" "$SDK/bin/thera"
cp -R "$ROOT/sdk/std" "$SDK/std"
VERSION="$("$SDK/bin/thera" --version | sed 's/^thera //')"
